...
const Button = class {
    ...
    render() {
        ...
        const iconStartEl = (index.h("calcite-icon", { class: { [CSS.icon]: true, [CSS.iconStart]: true }, flipRtl: this.iconFlipRtl === "start" || this.iconFlipRtl === "both", icon: this.iconStart, scale: component.getIconScale(this.scale) }));
        const iconEndEl = (index.h("calcite-icon", { class: { [CSS.icon]: true, [CSS.iconEnd]: true }, flipRtl: this.iconFlipRtl === "end" || this.iconFlipRtl === "both", icon: this.iconEnd, scale: component.getIconScale(this.scale) }));
        const contentEl = (index.h("span", { class: CSS.content, ref: (el) => (this.contentEl = el) }, index.h("slot", null)));
        return (index.h(interactive.InteractiveContainer, { disabled: this.disabled }, index.h(Tag, { "aria-disabled": childElType === "a" ? dom.toAriaBoolean(this.disabled || this.loading) : null, "aria-expanded": this.el.getAttribute("aria-expanded"), "aria-label": !this.loading ? label.getLabelText(this) : this.messages.loading, "aria-live": "polite", class: {
