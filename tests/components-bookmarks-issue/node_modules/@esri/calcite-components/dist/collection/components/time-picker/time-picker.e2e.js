...
describe("calcite-time-picker", () => {
    ...
    describe("focusing", () => {
        it("should focus input when corresponding nudge up button is clicked", async () => {
            ...
            const fractionalSecondElAriaLabel = (await page.find(`calcite-time-picker >>> .${CSS.fractionalSecond}`)).getAttribute("aria-label");
            ...
            const meridiemElAriaLabel = (await page.find(`calcite-time-picker >>> .${CSS.meridiem}`)).getAttribute("aria-label");
    ...
    describe("keyboard accessibility", () => {
        it("tabbing focuses each input in the correct sequence", async () => {
            ...
            await page.waitForChanges();
            expect(await page.$eval("calcite-time-picker", (element, selector) => element.shadowRoot.activeElement.matches(selector), `.${CSS.hour}`)).toBe(true);
            await page.keyboard.press("Tab");
            await page.waitForChanges();
            expect(await page.$eval("calcite-time-picker", (element, selector) => element.shadowRoot.activeElement.matches(selector), `.${CSS.minute}`)).toBe(true);
            await page.keyboard.press("Tab");
            await page.waitForChanges();
            expect(await page.$eval("calcite-time-picker", (element, selector) => element.shadowRoot.activeElement.matches(selector), `.${CSS.second}`)).toBe(true);
            await page.keyboard.press("Tab");
            await page.waitForChanges();
            expect(await page.$eval("calcite-time-picker", (element, selector) => element.shadowRoot.activeElement.matches(selector), `.${CSS.meridiem}`)).toBe(true);
        });
    ...
    describe("fractional second support", () => {
        it("upward nudge of empty fractional second sets to 0 for step=0.1", async () => {
            ...
            const [buttonUpLocationX, buttonUpLocationY] = await getElementXY(page, "calcite-time-picker", ".button--fractionalSecond-up");
...
//# sourceMappingURL=time-picker.e2e.js.map
