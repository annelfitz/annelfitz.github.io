...
import { accessible, defaults, disabled, floatingUIOwner, focusable, hidden, openClose, reflects, renders, } from "../../tests/commonTests";
...
describe("calcite-dropdown", () => {
  ...
  `;
    ...
    assertSelectedItems.setUpEvents = async (page) => {
        await page.evaluate(() => {
            document.addEventListener("calciteDropdownSelect", ({ detail }) => {
  ...
  `;
    ...
    describe("toggles the dropdown with click, enter, or space", () => {
      ...
      `);
            ...
            expect(await dropdownWrapper.isVisible()).toBe(false);
            await page.$eval("calcite-button[slot='trigger']", (triggerEl) => {
    ...
    `;
        ...
        await page.evaluate(async (templateHTML, wrapperName) => {
            ...
            wrapper.shadowRoot.querySelector("#item-3").click();
        }, wrappedDropdownTemplateHTML, wrapperName);
        await page.waitForChanges();
        const finalSelectedItem = await page.evaluate(async (wrapperName) => {
            const wrapper = document.querySelector(wrapperName);
    ...
    it.skip("dropdown should not overflow when wrapped inside a tab #3007", async () => {
        ...
        await button.click();
        await page.waitForChanges();
        expect(await page.$eval("calcite-dropdown", (dropdown) => {
    ...
    it("dropdown wrapper should have height when filter results empty and combined with a PickList in Panel  #3048", async () => {
        ...
        await page.waitForChanges();
        const dropdownContentHeight = await (await page.find("calcite-dropdown >>> .calcite-dropdown-wrapper")).getComputedStyle();
        await page.evaluate(() => {
