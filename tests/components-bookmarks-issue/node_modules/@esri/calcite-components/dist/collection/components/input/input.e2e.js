...
import { defaults, disabled, focusable, formAssociated, labelable, reflects, renders, hidden, t9n, } from "../../tests/commonTests";
...
describe("calcite-input", () => {
    ...
    it("renders number buttons in default vertical alignment when type=number", async () => {
        ...
        const numberHorizontalItemDown = await page.find("calcite-input >>> .number-button-item--horizontal[data-adjustment='down']");
        const numberHorizontalItemUp = await page.find("calcite-input >>> .number-button-item--horizontal[data-adjustment='up']");
    ...
    it("does not render number buttons in horizontal alignment when type=number, number button type is horizontal, and read-only", async () => {
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input type="number" number-button-type="horizontal" read-only></calcite-input>`);
        const numberHorizontalItemDown = await page.find("calcite-input >>> .number-button-item--horizontal[data-adjustment='down']");
        const numberHorizontalItemUp = await page.find("calcite-input >>> .number-button-item--horizontal[data-adjustment='up']");
        expect(numberHorizontalItemDown).toBeNull();
        expect(numberHorizontalItemUp).toBeNull();
    ...
    describe("input type number increment/decrement functionality", () => {
        ...
        it("correctly increments/decrements numbers greater than MAX_SAFE_INTEGER", async () => {
            await page.setContent(html `<calcite-input
        ...
        it("correctly increments and decrements on long hold on mousedown and step is set to a decimal", async () => {
            ...
            const [buttonUpLocationX, buttonUpLocationY] = await getElementXY(page, "calcite-input", ".number-button-item[data-adjustment='up']");
            ...
            const [buttonDownLocationX, buttonDownLocationY] = await getElementXY(page, "calcite-input", ".number-button-item[data-adjustment='down']");
    ...
    describe("emits events when value is modified", () => {
        async function assertChangeEvents(type) {
            const page = await newE2EPage();
    ...
    it("allows restricting input length", async () => {
        ...
        const getInputValidity = async () => page.$eval("calcite-input", (element) => {
    ...
    it("number input value stays in sync when value property is controlled with javascript", async () => {
        ...
        const input = await page.find("calcite-input >>> input");
        await page.evaluate(() => {
            document.querySelector("calcite-input").addEventListener("calciteInputInput", (event) => {
    ...
    describe("number locale support", () => {
        ...
        locales
            ...
            it(`displays group and decimal separator on initial load for ${locale} locale using opt-in prop`, async () => {
                const value = "1234.56";
                const page = await newE2EPage();
                await page.setContent(html `<calcite-input lang="${locale}" type="number" value="${value}" group-separator></calcite-input>`);
                const calciteInput = await page.find("calcite-input");
                const input = await page.find("calcite-input >>> input");
                numberStringFormatter.numberFormatOptions = {
                    locale,
            ...
            it(`displays correct formatted value when the value is changed programmatically for ${locale} locale`, async () => {
                const page = await newE2EPage();
                await page.setContent(html `<calcite-input lang="${locale}" type="number"></calcite-input><input id="external" />`);
                await page.evaluate(() => {
    ...
    it(`disallows pasting just text characters with no initial value`, async () => {
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input type="number"></calcite-input><input id="copy" value="invalid number" />`);
        const calciteInput = await page.find("calcite-input");
        const input = await page.find("calcite-input >>> input");
    ...
    it(`disallows pasting just text characters with existing number value`, async () => {
        ...
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input type="number" value="1234.56"></calcite-input><input id="copy" value="invalid number" />`);
        const calciteInput = await page.find("calcite-input");
        const input = await page.find("calcite-input >>> input");
        const copyInput = await page.find("#copy");
    ...
    it(`disallows pasting just text characters with no initial value with group separator`, async () => {
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input type="number" group-separator></calcite-input><input id="copy" value="invalid number" />`);
        const calciteInput = await page.find("calcite-input");
        const input = await page.find("calcite-input >>> input");
        const copyInput = await page.find("#copy");
        expect(await calciteInput.getProperty("value")).toBe("");
    ...
    it(`disallows pasting just text characters with existing number value with group separator`, async () => {
        const initialValue = "1234.56";
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input type="number" value="1234.56" group-separator></calcite-input
    ...
    describe("ArrowUp/ArrowDown function of moving caret to the beginning/end of text within calcite-input", () => {
        let page;
        const determineCaretIndex = (nestedInputTypeSelector, position) => {
            return page.evaluate((nestedInputTypeSelector, position) => {
                const element = document.querySelector("calcite-input");
        ...
        it("does not jump to the beginning of input while incrementing on ArrowUp held down on input type number", async () => {
            ...
            await page.keyboard.down("ArrowUp");
            await page.$eval("calcite-input", (element) => {
    ...
    it("allows disabling slotted action", async () => {
        const page = await newE2EPage();
        await page.setContent(`<calcite-input><calcite-button slot="action" disabled>Action</calcite-button></calcite-input>`);
        const input = await page.find("calcite-input");
