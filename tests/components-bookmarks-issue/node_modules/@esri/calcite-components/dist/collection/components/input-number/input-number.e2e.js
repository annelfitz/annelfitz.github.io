...
describe("calcite-input-number", () => {
     ...
     * @param numberAsText
     */
    async function typeNumberValue(page, numberAsText) {
    ...
    it("renders number buttons in default vertical alignment", async () => {
        ...
        const numberHorizontalItemDown = await page.find("calcite-input-number >>> .number-button-item--horizontal[data-adjustment='down']");
        const numberHorizontalItemUp = await page.find("calcite-input-number >>> .number-button-item--horizontal[data-adjustment='up']");
    ...
    it("does not render number buttons in horizontal alignment, number button type is horizontal, and read-only", async () => {
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input-number number-button-type="horizontal" read-only></calcite-input-number>`);
        const numberHorizontalItemDown = await page.find("calcite-input-number >>> .number-button-item--horizontal[data-adjustment='down']");
        const numberHorizontalItemUp = await page.find("calcite-input-number >>> .number-button-item--horizontal[data-adjustment='up']");
        expect(numberHorizontalItemDown).toBeNull();
        expect(numberHorizontalItemUp).toBeNull();
    ...
    describe.skip("increment/decrement functionality", () => {
        ...
        it("correctly increments/decrements numbers greater than MAX_SAFE_INTEGER", async () => {
            await page.setContent(html `<calcite-input-number
          value="100000000000000000000000000000000000000000000000000."
          step="10"
        ></calcite-input-number>`);
            const element = await page.find("calcite-input-number");
            const numberHorizontalItemDown = await page.find("calcite-input-number >>> .number-button-item[data-adjustment='down']");
            const numberHorizontalItemUp = await page.find("calcite-input-number >>> .number-button-item[data-adjustment='up']");
        ...
        it("correctly increments/decrements exponential notation numbers without losing precision", async () => {
            ...
            await numberHorizontalItemUp.click();
            await page.waitForChanges();
            expect(await element.getProperty("value")).toBe("1.00000000000000000000000000000000000000000000000000000000000123");
            element.setProperty("step", 0.1);
            await page.waitForChanges();
            Array.from({ length: 5 }, async () => await numberHorizontalItemDown.click());
            await page.waitForChanges();
            expect(await element.getProperty("value")).toBe("0.50000000000000000000000000000000000000000000000000000000000123");
        });
        ...
        it("correctly increments and decrements on long hold on mousedown and step is set to a decimal", async () => {
            ...
            const [buttonUpLocationX, buttonUpLocationY] = await getElementXY(page, "calcite-input-number", ".number-button-item[data-adjustment='up']");
            ...
            const [buttonDownLocationX, buttonDownLocationY] = await getElementXY(page, "calcite-input-number", ".number-button-item[data-adjustment='down']");
        ...
        describe("mouse events on arrow buttons", () => {
            ...
            describe("data-adjustment='down'", () => {
                ...
                beforeEach(async () => {
                    const [x, y] = await getElementXY(page, "calcite-input-number", ".number-button-item[data-adjustment='down']");
        ...
        it("when both 'ArrowUp' and 'ArrowDown' are pressed at the same time most recently pressed key takes over", async () => {
            ...
            await page.waitForChanges();
            await Promise.all(["ArrowUp", "ArrowDown"].map((key) => page.keyboard.press(key)));
            await page.waitForTimeout(delayFor2UpdatesInMs);
    ...
    describe("emits events when value is modified", () => {
        async function assertChangeEvents() {
            const page = await newE2EPage();
            await page.setContent(html `<calcite-input-number></calcite-input-number>`);
            const element = await page.find("calcite-input-number");
    ...
    it("value stays in sync when value property is controlled with javascript", async () => {
        ...
        await page.evaluate(() => {
            document
                ...
                .addEventListener("calciteInputNumberInput", (event) => {
    ...
    it("disallows typing non-numeric characters with shift modifier key down", async () => {
        ...
        for (let i = 0; i < nonELetterKeys.length; i++) {
            ...
            await page.keyboard.press(nonELetterKeys[i]);
    ...
    it("allows typing numeric characters with shift modifier key down (#6854)", async () => {
        ...
        for (let i = 0; i < numberKeysExcludingZero.length; i++) {
            await page.keyboard.down("Shift");
            await page.keyboard.press(numberKeysExcludingZero[i]);
    ...
    describe("number locale support", () => {
        ...
        locales
            ...
            it(`displays group and decimal separator on initial load for ${locale} locale using opt-in prop`, async () => {
                const value = "1234.56";
                const page = await newE2EPage();
                await page.setContent(html `<calcite-input-number lang="${locale}" value="${value}" group-separator></calcite-input-number>`);
                const calciteInput = await page.find("calcite-input-number");
                const input = await page.find("calcite-input-number >>> input");
                numberStringFormatter.numberFormatOptions = {
                    locale,
            ...
            it(`displays correct formatted value when the value is changed programmatically for ${locale} locale`, async () => {
                const page = await newE2EPage();
                await page.setContent(html `<calcite-input-number lang="${locale}"></calcite-input-number><input id="external" />`);
                await page.evaluate(() => {
    ...
    it(`disallows pasting just text characters with no initial value`, async () => {
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input-number></calcite-input-number><input id="copy" value="invalid number" />`);
        const calciteInput = await page.find("calcite-input-number");
        const input = await page.find("calcite-input-number >>> input");
    ...
    it(`disallows pasting just text characters with existing number value`, async () => {
        ...
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input-number value="1234.56"></calcite-input-number><input id="copy" value="invalid number" />`);
        const calciteInput = await page.find("calcite-input-number");
        const input = await page.find("calcite-input-number >>> input");
        const copyInput = await page.find("#copy");
    ...
    it(`disallows pasting just text characters with no initial value with group separator`, async () => {
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input-number group-separator></calcite-input-number><input id="copy" value="invalid number" />`);
        const calciteInput = await page.find("calcite-input-number");
        const input = await page.find("calcite-input-number >>> input");
        const copyInput = await page.find("#copy");
        expect(await calciteInput.getProperty("value")).toBe("");
    ...
    it(`disallows pasting just text characters with existing number value with group separator`, async () => {
        const initialValue = "1234.56";
        const page = await newE2EPage();
        await page.setContent(html `<calcite-input-number value="1234.56" group-separator></calcite-input-number
    ...
    describe("ArrowUp/ArrowDown function of moving caret to the beginning/end of text", () => {
        ...
        const determineCaretIndex = (position) => {
            return page.evaluate((position) => {
                const element = document.querySelector("calcite-input-number");
        ...
        it("does not jump to the beginning of input while incrementing on ArrowUp held down", async () => {
            ...
            await page.keyboard.down("ArrowUp");
            await page.$eval("calcite-input-number", (element) => {
    ...
    it("allows disabling slotted action", async () => {
        const page = await newE2EPage();
        await page.setContent(`<calcite-input-number><calcite-button slot="action" disabled>Action</calcite-button></calcite-input-number>`);
        const input = await page.find("calcite-input-number");
    ...
    it("should stop increasing the value when pointer is moved away from the increment button", async () => {
        ...
        await page.mouse.move(incrementButtonRect.left + incrementButtonRect.width / 2, incrementButtonRect.top + incrementButtonRect.height / 2);
        await page.mouse.down();
        await page.waitForChanges();
...
//# sourceMappingURL=input-number.e2e.js.map
