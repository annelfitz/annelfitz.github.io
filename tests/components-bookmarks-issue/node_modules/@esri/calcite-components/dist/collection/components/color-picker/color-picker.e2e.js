...
import { selectText, getElementXY, newProgrammaticE2EPage, toBeNumber, toBeInteger, } from "../../tests/utils";
...
describe("calcite-color-picker", () => {
    ...
    it("emits event when value changes via user interaction and not programmatically", async () => {
        ...
        const [centerColorFieldScopeX, centerColorFieldScopeY] = await getElementXY(page, "calcite-color-picker", `.${CSS.colorFieldScope}`);
        ...
        const [draggedColorFieldScopeX, draggedColorFieldScopeY] = await getElementXY(page, "calcite-color-picker", `.${CSS.colorFieldScope}`);
    ...
    describe("color format", () => {
        ...
        it("changing format updates value", async () => {
            const page = await newE2EPage();
            await page.setContent(`<calcite-color-picker value=${supportedFormatToSampleValue["hex"]}></calcite-color-picker>`);
            const color = await page.find("calcite-color-picker");
            for (const format in supportedFormatToSampleValue) {
                color.setProperty("format", format);
                await page.waitForChanges();
    ...
    it("allows selecting colors via color field/slider", async () => {
        ...
        expect(spy).toHaveReceivedEventTimes(++changes);
        await page.mouse.click(colorFieldX + mediumScaleDimensions.colorField.width - widthOffset, colorFieldY + mediumScaleDimensions.colorField.height - 0.1);
        await page.waitForChanges();
        expect(await picker.getProperty("value")).toBe("#000000");
        expect(spy).toHaveReceivedEventTimes(++changes);
    ...
    describe("color inputs", () => {
        it("numbering system does not revert to latn when clamping RGB channels", async () => {
            const page = await newE2EPage();
            await page.setContent(html `<calcite-color-picker numbering-system="arab" value="#fff000"></calcite-color-picker>`);
            ...
            await page.waitForChanges();
            const nativeInputValue = await page.evaluate(async (CSS) => document
                .querySelector("calcite-color-picker")
                .shadowRoot.querySelector(`.${CSS.channel}`)
                .shadowRoot.querySelector("input").value, CSS);
        ...
        describe("default", () => {
            describe("keeps value in same format when applying updates", () => {
                ...
                const updateColorWithAllInputs = async (page, assertColorUpdate) => {
                    const hexInput = await page.find(`calcite-color-picker >>> calcite-color-picker-hex-input`);
                    ...
                    const [rInput, gInput, bInput, hInput, sInput, vInput] = await page.findAll(`calcite-color-picker >>> .${CSS.channel}`);
                ...
                it("supports hex", async () => {
                    ...
                    await page.waitForChanges();
                    await updateColorWithAllInputs(page, (value) => {
            ...
            describe("color gets propagated to support inputs", () => {
                ...
                describe("when no-color", () => {
                    ...
                    it("changes the value to the specified format after being empty", async () => {
                        const page = await newE2EPage();
                        await page.setContent(html `<calcite-color-picker allow-empty value="" format="rgb"></calcite-color-picker>`);
                        const color = await page.find("calcite-color-picker");
                        const hexInput = await page.find(`calcite-color-picker >>> calcite-color-picker-hex-input`);
        ...
        describe("alpha channel", () => {
            describe("keeps value in alpha-compatible format when applying updates", () => {
                ...
                const updateColorWithAllInputs = async (page, assertColorUpdate) => {
                    ...
                    const [rgbModeButton, hsvModeButton] = await page.findAll(`calcite-color-picker >>> .${CSS.colorMode}`);
                    const [rInput, gInput, bInput, rgbAInput, hInput, sInput, vInput, hsvAInput] = await page.findAll(`calcite-color-picker >>> .${CSS.channel}`);
                    await rgbModeButton.click();
                    await clearAndEnterHexOrChannelValue(page, rInput, "128");
                    await clearAndEnterHexOrChannelValue(page, gInput, "64");
                    await clearAndEnterHexOrChannelValue(page, bInput, "32");
                ...
                it("supports hex", async () => {
                    ...
                    picker.setProperty("value", hex);
                    await page.waitForChanges();
                    await updateColorWithAllInputs(page, async (value) => {
                        expect(value).not.toBe(hex);
            ...
            describe("color gets propagated to support inputs", () => {
                describe("valid color", () => {
                    ...
                    describe("allows modifying color via hex, RGBA, HSVA inputs", () => {
                        ...
                        it("allows modifying color via RGBA inputs", async () => {
                            const [rgbModeButton] = await page.findAll(`calcite-color-picker >>> .${CSS.colorMode}`);
                            const [rInput, gInput, bInput, rgbAInput] = await page.findAll(`calcite-color-picker >>> .${CSS.channel}`);
                            await rgbModeButton.click();
                            await clearAndEnterHexOrChannelValue(page, rInput, "128");
                            await clearAndEnterHexOrChannelValue(page, gInput, "64");
                            await clearAndEnterHexOrChannelValue(page, bInput, "32");
                        ...
                        it("allows modifying color via HSVA inputs", async () => {
                            const [, hsvModeButton] = await page.findAll(`calcite-color-picker >>> .${CSS.colorMode}`);
                            const [, , , , hInput, sInput, vInput, hsvAInput] = await page.findAll(`calcite-color-picker >>> .${CSS.channel}`);
                            await hsvModeButton.click();
                            // modifying value channel first to ensure other channel changes affect the underlying color
                            await clearAndEnterHexOrChannelValue(page, vInput, "45");
                            await clearAndEnterHexOrChannelValue(page, hInput, "180");
                    ...
                    describe("allows nudging values", () => {
                        ...
                        beforeEach(async () => {
                            page = await newE2EPage();
                            await page.setContent(html `<calcite-color-picker alpha-channel value="#40804880"></calcite-color-picker>`);
                            changeEventSpy = await page.spyOnEvent("calciteColorPickerChange");
                        });
                    ...
                    const assertChannelValueNudge = async (page, calciteInputOrSlider) => {
                        ...
                        function nudgeValue(value, amount) {
                ...
                describe("when no-color", () => {
                    it("color gets propagated to hex, RGB, HSV & opacity inputs", async () => {
                        const page = await newE2EPage();
                        await page.setContent(html `<calcite-color-picker alpha-channel allow-empty value=""></calcite-color-picker>`);
                        const hexInput = await page.find(`calcite-color-picker >>> calcite-color-picker-hex-input`);
                        expect(await hexInput.getProperty("value")).toBe(null);
                        const [rgbModeButton, hsvModeButton] = await page.findAll(`calcite-color-picker >>> .${CSS.colorMode}`);
                        const [rInput, gInput, bInput, rgbAInput, hInput, sInput, vInput, hsvAInput] = await page.findAll(`calcite-color-picker >>> .${CSS.channel}`);
                    ...
                    describe("restores previous color value when a nudge key is pressed", () => {
                        ...
                        beforeEach(async () => {
                            page = await newE2EPage();
                            await page.setContent(`<calcite-color-picker alpha-channel allow-empty value='${initialValue}'></calcite-color-picker>`);
                        });
                        ...
                        const assertChannelValueNudge = async (page, calciteInputOrSlider, customValueClearingFn) => {
                            ...
                            await page.keyboard.press("ArrowUp");
                            await page.waitForChanges();
                            expect(await calciteInputOrSlider.getProperty("value")).toBe(ensureValueType(consistentRgbHsvChannelValue));
                            await clearValue();
                            await page.keyboard.press("ArrowDown");
                            await page.waitForChanges();
                            expect(await calciteInputOrSlider.getProperty("value")).toBe(ensureValueType(consistentRgbHsvChannelValue));
                    ...
                    it("changes the value to the specified format after being empty", async () => {
                        const page = await newE2EPage();
                        await page.setContent("<calcite-color-picker alpha-channel allow-empty value='' format='rgba'></calcite-color-picker>");
                        const color = await page.find("calcite-color-picker");
                        const hexInput = await page.find(`calcite-color-picker >>> calcite-color-picker-hex-input`);
                    ...
                    describe("clearing color via supporting inputs", () => {
                        it("clears color via hex input", async () => {
                            const page = await newE2EPage();
                            await page.setContent("<calcite-color-picker alpha-channel allow-empty value='#c0ff3333'></calcite-color-picker>");
                            const picker = await page.find("calcite-color-picker");
                            const hexInput = await page.find(`calcite-color-picker >>> calcite-color-picker-hex-input`);
                            await clearAndEnterHexOrChannelValue(page, hexInput, "");
                            expect(await picker.getProperty("value")).toBe(null);
...
//# sourceMappingURL=color-picker.e2e.js.map
