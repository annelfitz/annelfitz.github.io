...
describe("calcite-shell-panel", () => {
    ...
    it("has a slot", async () => {
        ...
        const contentBodyHasSlot = await page.$eval("calcite-shell-panel", (panel, contentBodyClass) => {
    ...
    it("should show panel content", async () => {
        const page = await newE2EPage();
        await page.setContent('<calcite-shell-panel><div slot="action-bar">bar</div><div>content</div></calcite-shell-panel>');
    ...
    it("collapsed property should hide panel content", async () => {
        const page = await newE2EPage();
        await page.setContent('<calcite-shell-panel collapsed><div slot="action-bar">bar</div><div>content</div></calcite-shell-panel>');
        await page.waitForChanges();
        const element = await page.find(`calcite-shell-panel >>> .${CSS.content}`);
        const isVisible = await element.isVisible();
    ...
    it("start position property should have action slot first", async () => {
        const page = await newE2EPage();
        await page.setContent('<calcite-shell-panel position="start"><div slot="action-bar">bar</div><div>content</div></calcite-shell-panel>');
        const actionSlotIsFirst = await page.$eval("calcite-shell-panel", (panel, containerClass, slotName) => {
            const container = panel.shadowRoot.querySelector(containerClass);
            return (container.firstElementChild.tagName == "SLOT" &&
                container.firstElementChild.name == slotName);
        }, `.${CSS.container}`, SLOTS.actionBar);
        expect(actionSlotIsFirst).toBe(true);
    });
    it("trailing position property should have DIV first", async () => {
        const page = await newE2EPage();
        await page.setContent('<calcite-shell-panel position="end"><div slot="action-bar">bar</div><div>content</div></calcite-shell-panel>');
        const divElementIsFirst = await page.$eval("calcite-shell-panel", (panel, containerClass, contentClass) => {
            const container = panel.shadowRoot.querySelector(containerClass);
            return container.firstElementChild.className == contentClass;
        }, `.${CSS.container}`, CSS.content);
    ...
    it("should have correct animation class when overlaid, layout vertical, and rtl, position start", async () => {
        const page = await newE2EPage();
        await page.setContent("<div dir='rtl'><calcite-shell-panel position='start'><div>content</div></calcite-shell-panel></div>");
        const panel = await page.find("calcite-shell-panel");
        panel.setProperty("displayMode", "overlay");
        await page.waitForChanges();
        const animationClassRight = await page.find(`calcite-shell-panel >>> .${CSS_UTILITY.calciteAnimateInRight}`);
    ...
    `);
        await page.waitForChanges();
        let separator = await page.find(`calcite-shell-panel >>> .${CSS.separator}`);
    ...
    `);
        await page.waitForChanges();
        const separator = await page.find(`calcite-shell-panel >>> .${CSS.separator}`);
        const content = await page.find(`calcite-shell-panel >>> .${CSS.content}`);
        expect(separator).toBeDefined();
        expect(content).toBeDefined();
        expect(separator.getAttribute("aria-valuenow")).toBe("320");
...
//# sourceMappingURL=shell-panel.e2e.js.map
