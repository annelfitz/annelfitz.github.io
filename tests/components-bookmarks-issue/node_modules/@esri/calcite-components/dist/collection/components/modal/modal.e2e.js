...
describe("calcite-modal", () => {
    ...
    it("does not overflow page bounds when requested css variable sizes are larger than viewport", async () => {
        ...
        await page.setContent(`<calcite-modal style="--calcite-modal-height:1200px;--calcite-modal-width:1200px;" open></calcite-modal>`);
        const modal = await page.find("calcite-modal");
        await modal.setProperty("open", true);
        await page.waitForChanges();
    ...
    `);
        const modal = await page.find("calcite-modal");
        await page.$eval("calcite-modal", (el) => (el.beforeClose = window.beforeClose));
        await page.waitForChanges();
  ...
  `);
    ...
    it("should handle rejected 'beforeClose' promise'", async () => {
        ...
        await page.$eval("calcite-modal", (elm) => (elm.beforeClose = window.beforeClose));
        const modal = await page.find("calcite-modal");
    ...
    describe("calcite-modal accessibility checks", () => {
        it("traps focus within the modal when open", async () => {
            ...
            const page = await newE2EPage();
            await page.setContent(html `<calcite-modal>
          <div slot="content">
      ...
      `);
        ...
        it("traps focus within the modal when open and disabled close button", async () => {
            ...
            const button2Id = "button2";
            const page = await newE2EPage();
            await page.setContent(html `<calcite-modal close-button-disabled>
          <div slot="content">
            <button id="${button1Id}">Focus1</button>
            <button id="${button2Id}">Focus2</button>
          </div>
    ...
    describe("setFocus", () => {
        const createModalHTML = (contentHTML, attrs) => `<calcite-modal open ${attrs}>${contentHTML}</calcite-modal>`;
    ...
    describe("overflow prevention", () => {
        async function hasOverflowStyle(page) {
