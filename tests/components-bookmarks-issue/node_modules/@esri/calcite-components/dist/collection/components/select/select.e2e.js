...
import { accessible, disabled, defaults, focusable, formAssociated, labelable, reflects, renders, hidden, } from "../../tests/commonTests";
...
describe("calcite-select", () => {
  ...
  `;
    ...
    async function assertSelectedOption(page, selectedOption) {
        const selectedOptionValue = await page.$eval("calcite-select", (select) => select.selectedOption.value);
    ...
    describe("flat options", () => {
        ...
        `,
            ...
            const internalSelect = await page.evaluateHandle(() => document.querySelector("calcite-select").shadowRoot.querySelector("select"));
      ...
      `,
        ...
        await page.evaluate(() => {
            document.querySelector("calcite-select").addEventListener("calciteSelectChange", (event) => {
                window.selectedOptionId = event.target.querySelector("calcite-option[selected]").id;
            });
        });
        const internalSelect = await page.evaluateHandle(() => document.querySelector("calcite-select").shadowRoot.querySelector("select"));
        await internalSelect.asElement().select("dos");
        await page.waitForChanges();
        const selectedOptionId = await page.evaluate(() => window.selectedOptionId);
    ...
    `);
        await page.$eval("calcite-select", (select) => select.addEventListener("calciteSelectChange", (event) => {
            window.selectedOptionId = event.target.querySelector("calcite-option[selected]").id;
        }));
        const internalSelect = await page.evaluateHandle(() => document.querySelector("calcite-select").shadowRoot.querySelector("select"));
