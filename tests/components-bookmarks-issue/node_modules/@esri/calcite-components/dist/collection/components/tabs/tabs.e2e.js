...
describe("calcite-tabs", () => {
  ...
  `;
    ...
    `);
        await page.$eval("calcite-tabs", (element) => {
    ...
    function testTabsScaleAndPosition(scale, position) {
    ...
    `);
        await page.evaluate(() => document.addEventListener("calciteTabChange", (event) => (window.selectedTitleTab = event.target.selectedTitle.tab), { once: true }));
        const tabChange = page.waitForEvent("calciteTabChange");
        await page.click("calcite-tab-title");
        await tabChange;
        const selectedTitleOnEmit = await page.evaluate(() => window.selectedTitleTab);
