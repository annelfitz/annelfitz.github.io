...
import { dragAndDrop, isElementFocused, getFocusedElementProp } from "../../tests/utils";
...
describe("calcite-list", () => {
    ...
    describe("accessible", () => {
        ...
        accessible(html `<calcite-list filter-enabled filter-text="Bananas" selection-appearance="border" selection-mode="single">
    ...
    describe("disabled", () => {
        ...
        <calcite-list-item label="test" description="hello world"></calcite-list-item>
      </calcite-list>`, { focusTarget: "child" });
    ...
    `);
        await page.waitForChanges();
        async function getSelectedItemValues() {
            return await page.$eval("calcite-list", (list) => list.selectedItems.map((item) => item.value));
        }
        const list = await page.find("calcite-list");
    ...
    it("should support shift click to select multiple items", async () => {
        const clickItemContent = (item, selector) => {
    ...
    `);
        ...
        const listItemOneContentContainer = await page.find(`calcite-list-item[value=one] >>> .${ListItemCSS.contentContainer}`);
    ...
    it("should show no-results content when filter does not match", async () => {
        const page = await newE2EPage();
        await page.setContent(html `<calcite-list>
        <calcite-list-item label="Apples" value="apples"></calcite-list-item>
        <calcite-list-item label="Oranges" value="oranges"></calcite-list-item>
        <calcite-list-item label="Pears" value="pears"></calcite-list-item>
        <calcite-notice slot="filter-no-results" icon kind="warning" scale="s" open>
    ...
    describe("drag and drop", () => {
        async function createSimpleList() {
            const page = await newE2EPage();
      ...
      `);
            ...
            await page.evaluate(() => {
                ...
                lists.forEach((list) => list.addEventListener("calciteListOrderChange", () => {
                    testWindow.calledTimes++;
                }));
            });
            await dragAndDrop(page, {
            ...
            const [first, second, third, fourth, fifth, sixth, seventh, eight, ninth] = await page.findAll("calcite-list-item");
            ...
            expect(await page.evaluate(() => window.calledTimes)).toBe(2);
        });
        it("works using a keyboard", async () => {
            ...
            async function assertKeyboardMove(arrowKey, expectedValueOrder, newIndex, oldIndex) {
                await page.waitForChanges();
        ...
        it("is drag and drop list accessible", async () => {
            ...
            async function getAriaLabel() {
                return page.$eval("calcite-list-item[value='one']", (el) => {
            ...
            function messageSubstitute({ text, setPosition, label, setSize, }) {
            ...
            expect(handleAriaLabel).toBe(messageSubstitute({
            ...
            expect(await handle.getProperty("selected")).toBe(true);
            await page.waitForChanges();
            expect(assistiveTextElement.textContent).toBe(messageSubstitute({
            ...
            expect(changeHandleLabel).toBe(messageSubstitute({
...
//# sourceMappingURL=list.e2e.js.map
