 ...
 * @param item
 */
async function directItemClick(page, item) {
...
describe("calcite-tree", () => {
      ...
      `);
        ...
        describe("has selected items in the selection event payload", () => {
            ...
            it("contains current selection when selection=multichildren", async () => {
                const page = await newE2EPage();
                await page.setContent(html `<calcite-tree lines selection-mode="multichildren" scale="s">
        ...
        describe(`when tree-item selection-mode is "ancestors"`, () => {
          ...
          `,
                ...
                const checkbox = await page.find(`calcite-tree-item >>> .${CSS.nodeContainer} .${CSS.checkboxLabel} .${CSS.checkbox}`);
    ...
    describe("keyboard support", () => {
        ...
        it("honors disabled items when navigating the tree", async () => {
            const page = await newE2EPage();
            await page.setContent(html ` <calcite-tree selection-mode="ancestors" id="root">
    ...
    describe("not throwing if tree doesn't have a parent element on initial render (#5333)", () => {
        ...
        it("does not throw when tree is the topmost element in a shadow root", async () => {
            ...
            await page.evaluate(async () => {
                customElements.define("test-tree-element", class extends HTMLElement {
                    constructor() {
                        super();
    ...
    describe("parent node expansion", () => {
        describe("keyboard", () => {
            assertSelectionModes(false, async (_page, item) => {
        ...
        describe("mouse", () => {
            assertSelectionModes(true, async (page, item) => {
        ...
        async function assertSelectionModes(childToggleTraversesParent, selectItem, toggleItem, selectItemChild) {
            const selectionModesTests = [
                {
            ...
            selectionModesTests.forEach(({ selectionMode, canDeselect, expandableItemClick: { selectsItem, selectsChildren } }) => {
            ...
            `);
                    ...
                    expect(await expandableParentItem.getProperty("expanded")).toBe(!selectsItem && !childToggleTraversesParent);
                    expect(await tree.getProperty("selectedItems")).toHaveLength(selectionMode === "none"
                        ? 0
