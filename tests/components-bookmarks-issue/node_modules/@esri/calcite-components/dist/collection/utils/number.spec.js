...
import { BigDecimal, addLocalizedTrailingDecimalZeros, expandExponentialNumberString, isValidNumber, parseNumberString, sanitizeNumberString, } from "./number";
...
describe("BigDecimal", () => {
    ...
    locales.forEach((locale) => {
        it(`correctly localizes number parts - ${locale}`, () => {
            ...
            expect(groupPart.trim().length === 0 || groupPart === " " ? "\u00A0" : groupPart).toBe(numberStringFormatter.group);
...
describe("expandExponentialNumberString", () => {
    ...
    it("decimal based exponential number strings", () => {
        ...
        expect(expandExponentialNumberString("1.2345678987654321000000000000000000000000000000000000000000e16")).toBe("12345678987654321");
    });
    it("exponential number strings with negative magnitude", () => {
        expect(expandExponentialNumberString("1.23e-60")).toBe("0.00000000000000000000000000000000000000000000000000000000000123");
    ...
    it("handles non-exponential numbers", () => {
        expect(expandExponentialNumberString("1100000000000000000000000000000000000000000000000000")).toBe("1100000000000000000000000000000000000000000000000000");
        expect(expandExponentialNumberString("")).toBe("");
    });
});
describe("addLocalizedTrailingDecimalZeros", () => {
    function getLocalizedDecimalValue(value, trailingZeros) {
    ...
    locales.forEach((locale) => {
        it(`add back sanitized trailing decimal zero values - ${locale}`, () => {
            ...
            const bigDecimalWithTrailingZeros = "1230000000000000000000000000000.00000000000000000000045000000000000000000000000";
            const negativeExponentialString = "-10.021e10000";
            expect(addLocalizedTrailingDecimalZeros(numberStringFormatter.localize(stringWithTrailingZeros), stringWithTrailingZeros, numberStringFormatter)).toBe(getLocalizedDecimalValue(stringWithTrailingZeros, 3));
            expect(addLocalizedTrailingDecimalZeros(numberStringFormatter.localize(bigDecimalWithTrailingZeros), bigDecimalWithTrailingZeros, numberStringFormatter)).toBe(getLocalizedDecimalValue(bigDecimalWithTrailingZeros, 24));
            expect(addLocalizedTrailingDecimalZeros(numberStringFormatter.localize(negativeExponentialString), negativeExponentialString, numberStringFormatter)).toBe(numberStringFormatter.localize(negativeExponentialString));
        });
