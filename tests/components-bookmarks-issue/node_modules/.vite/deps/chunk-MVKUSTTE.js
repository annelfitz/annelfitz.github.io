...
var _ = class extends ii {
  ...
  _afterNew() {
    super._afterNew(), this._data.incrementRef(), this._updateFields(), this._disposers.push(this.data.events.onAll((e2) => {
      ...
      if (this.markDirtyValues(), this._markDirtyGroup(), this._dataChanged = true, "clear" === e2.type)
        ...
        const s = new g2(this, e2.newValue, this._makeDataItem(e2.newValue));
        t.push(s), this.processDataItem(s);
      } else if ("setIndex" === e2.type) {
        const s = t[e2.index], i = this._makeDataItem(e2.newValue);
        s.bullets && 0 == s.bullets.length && (s.bullets = void 0), N(i).forEach((e3) => {
  ...
  _updateFields() {
    this.valueFields && (this._valueFields = [], this._valueFieldsF = {}, w(this.valueFields, (e2) => {
      this.get(e2 + "Field") && (this._valueFields.push(e2), this._valueFieldsF[e2] = { fieldKey: e2 + "Field", workingKey: e2 + "Working" });
    })), this.fields && (this._fields = [], this._fieldsF = {}, w(this.fields, (e2) => {
...
var y2 = { millisecond: 1, second: 1e3, minute: 6e4, hour: 36e5, day: 864e5, week: 6048e5, month: 2629742400, year: 31536e6 };
function w2(e2, t) {
  return null == t && (t = 1), y2[e2] * t;
}
function I(e2, t, s, i, a, l, n2) {
    ...
    switch (a || "millisecond" == t || (n3 = e2.getTimezoneOffset(), e2.setUTCMinutes(e2.getUTCMinutes() - n3)), t) {
      case "day":
        ...
        if (s > 1) {
          if (l) {
            l = I(l, "day", 1);
            let t3 = e2.getTime() - l.getTime(), i2 = Math.floor(t3 / w2("day") / s), a3 = w2("day", i2 * s);
            e2.setTime(l.getTime() + a3 - n3 * w2("minute"));
          }
        } else
          e2.setUTCDate(t2);
      ...
      case "week":
        if (s > 1 && l) {
          l = I(l, "week", 1);
          let t3 = e2.getTime() - l.getTime(), i2 = Math.floor(t3 / w2("week") / s), a3 = w2("week", i2 * s);
          e2.setTime(l.getTime() + a3 - n3 * w2("minute"));
        }
        let m = e2.getUTCDate(), p = e2.getUTCDay();
        g(i) || (i = 1), m = p >= i ? m - p + i : m - (7 + p) + i, e2.setUTCDate(m), e2.setUTCHours(0, 0, 0, 0);
    }
  ...
  }
  {
    if (isNaN(e2.getTime()))
      return e2;
    let o = n2.offsetUTC(e2), h = e2.getTimezoneOffset(), u = n2.parseDate(e2), d = u.year, c = u.month, m = u.day, p = u.hour, f = u.minute, b2 = u.second, g3 = u.millisecond, _2 = u.weekday, v2 = o - h;
    switch (t) {
      case "day":
        if (s > 1 && l) {
          l = I(l, "day", 1, i, a, void 0, n2);
          let t2 = e2.getTime() - l.getTime(), r2 = Math.floor(t2 / w2("day") / s), o2 = w2("day", r2 * s);
          e2.setTime(l.getTime() + o2), u = n2.parseDate(e2), d = u.year, c = u.month, m = u.day;
        }
        p = 0, f = v2, b2 = 0, g3 = 0;
        break;
      case "second":
        f += v2, s > 1 && (b2 = Math.floor(b2 / s) * s), g3 = 0;
        break;
      case "millisecond":
        f += v2, s > 1 && (g3 = Math.floor(g3 / s) * s);
        break;
      case "hour":
        s > 1 && (p = Math.floor(p / s) * s), f = v2, b2 = 0, g3 = 0;
        break;
      case "minute":
        s > 1 && (f = Math.floor(f / s) * s), f += v2, b2 = 0, g3 = 0;
        break;
      case "month":
        s > 1 && (c = Math.floor(c / s) * s), m = 1, p = 0, f = v2, b2 = 0, g3 = 0;
        break;
      case "year":
        s > 1 && (d = Math.floor(d / s) * s), c = 0, m = 1, p = 0, f = v2, b2 = 0, g3 = 0;
        break;
      case "week":
        g(i) || (i = 1), m = _2 >= i ? m - _2 + i : m - (7 + _2) + i, p = 0, f = v2, b2 = 0, g3 = 0;
    }
    let y3 = (e2 = new Date(d, c, m, p, f, b2, g3)).getTimezoneOffset(), D2 = n2.offsetUTC(e2) - y3;
...
var D = class extends _ {
  ...
  _calculateAggregates(e2, t) {
    ...
    w(s, (e3) => {
      ...
      let a = s2 + "Change", m = s2 + "ChangePercent", p = s2 + "ChangePrevious", f = s2 + "ChangePreviousPercent", b2 = s2 + "ChangeSelection", g3 = s2 + "ChangeSelectionPercent", _2 = "valueY";
      "valueX" != s2 && "openValueX" != s2 && "lowValueX" != s2 && "highValueX" != s2 || (_2 = "valueX");
      const v2 = this.getPrivate("baseValueSeries"), y3 = this.getPrivate("adjustedStartIndex", e2);
      for (let w3 = y3; w3 < t; w3++) {
        const t2 = this.dataItems[w3];
        let y4 = t2.get(s2);
        null != y4 && (r2[s2]++, i[s2] += y4, l[s2] += Math.abs(y4), d[s2] = i[s2] / r2[s2], (n2[s2] > y4 || null == n2[s2]) && (n2[s2] = y4), (o[s2] < y4 || null == o[s2]) && (o[s2] = y4), u[s2] = y4, null == h[s2] && (h[s2] = y4, c[s2] = y4, v2 && (h[_2] = v2._getBase(_2))), 0 === e2 && (t2.setRaw(a, y4 - h[_2]), t2.setRaw(m, (y4 - h[_2]) / h[_2] * 100)), t2.setRaw(p, y4 - c[_2]), t2.setRaw(f, (y4 - c[_2]) / c[_2] * 100), t2.setRaw(b2, y4 - h[_2]), t2.setRaw(g3, (y4 - h[_2]) / h[_2] * 100), c[s2] = y4);
      }
      if (t < this.dataItems.length - 1) {
        const e3 = this.dataItems[t];
        let i2 = e3.get(s2);
        e3.setRaw(p, i2 - c[_2]), e3.setRaw(f, (i2 - c[_2]) / c[_2] * 100), e3.setRaw(b2, i2 - h[_2]), e3.setRaw(g3, (i2 - h[_2]) / h[_2] * 100);
      }
      e2 > 0 && e2--, delete c[s2];
      for (let t2 = e2; t2 < y3; t2++) {
        const e3 = this.dataItems[t2];
        let i2 = e3.get(s2);
        null == c[s2] && (c[s2] = i2), null != i2 && (e3.setRaw(p, i2 - c[_2]), e3.setRaw(f, (i2 - c[_2]) / c[_2] * 100), e3.setRaw(b2, i2 - h[_2]), e3.setRaw(g3, (i2 - h[_2]) / h[_2] * 100), c[s2] = i2);
      }
    }), w(s, (e3) => {
    ...
    }), 0 === e2 && t === this.dataItems.length && w(s, (e3) => {
    ...
    if (this._valuesDirty && e2 && e2.length > 0 && w(e2, (e3) => {
      ...
      w(e3.target._entities, (i) => {
        ...
        if (!g(a)) {
          ...
          if (!e3.customFunction)
            return;
        }
        if (e3.customFunction)
          e3.customFunction.call(this, i, t, s, a);
        else {
          let l, n2;
          l = e3.logarithmic ? (Math.log(a) * Math.LOG10E - Math.log(t) * Math.LOG10E) / (Math.log(s) * Math.LOG10E - Math.log(t) * Math.LOG10E) : (a - t) / (s - t), !g(a) || g(l) && Math.abs(l) != 1 / 0 || (l = 0.5), g(e3.min) ? n2 = e3.min + (e3.max - e3.min) * l : e3.min instanceof zt ? n2 = zt.interpolate(l, e3.min, e3.max) : e3.min instanceof e && (n2 = Zt(l, e3.min, e3.max)), i.set(e3.key, n2);
          const o = i.states;
          if (o) {
  ...
  _sequencedShowHide(t, s) {
    return n(this, void 0, void 0, function* () {
      if (this.get("sequencedInterpolation"))
        if (g(s) || (s = this.get("interpolationDuration", 0)), s > 0) {
          const i = this.startIndex(), a = this.endIndex();
          yield Promise.all(y(this.dataItems, (l, r2) => n(this, void 0, void 0, function* () {
            let e2 = s || 0;
          ...
          yield Promise.all(y(this.dataItems, (e2) => t ? this.showDataItem(e2, 0) : this.hideDataItem(e2, 0)));
    });
  }
  ...
  arcTo(e2, t, s, i, a) {
    ...
    if (null === this._x1)
      ...
      if (Math.abs(u * n2 - o * h) > O && a) {
        let c = s - l, m = i - r2, p = n2 * n2 + o * o, f = c * c + m * m, b2 = Math.sqrt(p), g3 = Math.sqrt(d), _2 = a * Math.tan((T - Math.acos((p + d - f) / (2 * b2 * g3))) / 2), v2 = _2 / g3, y3 = _2 / b2;
        Math.abs(v2 - 1) > O && this._append`L${e2 + v2 * h},${t + v2 * u}`, this._append`A${a},${a},0,0,${+(u * c > h * m)},${this._x1 = e2 + y3 * n2},${this._y1 = t + y3 * o}`;
      } else
        this._append`L${this._x1 = e2},${this._y1 = t}`;
    else
      ;
...
Object.defineProperty(N2, "className", { enumerable: true, configurable: true, writable: true, value: "Chart" }), Object.defineProperty(N2, "classNames", { enumerable: true, configurable: true, writable: true, value: ii.classNames.concat([N2.className]) });
...
Object.defineProperty(R, "className", { enumerable: true, configurable: true, writable: true, value: "SerialChart" }), Object.defineProperty(R, "classNames", { enumerable: true, configurable: true, writable: true, value: N2.classNames.concat([R.className]) });
...
//# sourceMappingURL=chunk-MVKUSTTE.js.map
