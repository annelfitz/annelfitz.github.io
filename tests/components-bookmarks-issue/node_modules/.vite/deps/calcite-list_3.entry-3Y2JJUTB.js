...
var listSelector$1 = "calcite-list";
var listItemGroupSelector = "calcite-list-item-group";
var listItemSelector$1 = "calcite-list-item";
...
function getListItemChildren(slotEl) {
  ...
  const listItemGroupChildren = assignedElements.filter((el) => el == null ? void 0 : el.matches(listItemGroupSelector)).map((group) => Array.from(group.querySelectorAll(listItemSelector$1))).reduce((previousValue, currentValue) => [...previousValue, ...currentValue], []);
  const listItemChildren = assignedElements.filter((el) => el == null ? void 0 : el.matches(listItemSelector$1));
  const listItemListChildren = assignedElements.filter((el) => el == null ? void 0 : el.matches(listSelector$1)).map((list) => Array.from(list.querySelectorAll(listItemSelector$1))).reduce((previousValue, currentValue) => [...previousValue, ...currentValue], []);
...
var List = class {
  ...
  render() {
    ...
    return h(InteractiveContainer, { disabled: this.disabled }, h("div", { class: CSS$1.container }, this.dragEnabled ? h("span", { "aria-live": "assertive", class: CSS$1.assistiveText }, this.assistiveText) : null, this.renderItemAriaLive(), loading ? h("calcite-scrim", { class: CSS$1.scrim, loading }) : null, h("table", { "aria-busy": toAriaBoolean(loading), "aria-label": label || "", class: CSS$1.table, onKeyDown: this.handleListKeydown, role: "treegrid" }, filterEnabled || hasFilterActionsStart || hasFilterActionsEnd ? h("thead", null, h("tr", { class: { [CSS$1.sticky]: true } }, h("th", { colSpan: MAX_COLUMNS }, h("calcite-stack", { class: CSS$1.stack }, h("slot", { name: SLOTS2.filterActionsStart, onSlotchange: this.handleFilterActionsStartSlotChange, slot: SLOTS.actionsStart }), h("calcite-filter", {
    ...
    }), h("slot", { name: SLOTS2.filterActionsEnd, onSlotchange: this.handleFilterActionsEndSlotChange, slot: SLOTS.actionsEnd }))))) : null, h("tbody", { class: CSS$1.tableContainer }, h("slot", { onSlotchange: this.handleDefaultSlotChange }))), h("div", { "aria-live": "polite", "data-test-id": "no-results-container", hidden: !(hasFilterNoResults && filterEnabled && filterText && !filteredItems.length) }, h("slot", { name: SLOTS2.filterNoResults, onSlotchange: this.handleFilterNoResultsSlotChange }))));
  }
  // --------------------------------------------------------------------------
  //
  //  Private Methods
  ...
  renderItemAriaLive() {
    ...
    return !parentListEl ? h("div", { "aria-live": "polite", class: CSS$1.assistiveText }, filterEnabled && filterText && (filteredData == null ? void 0 : filteredData.length) ? h("div", { key: "aria-filter-enabled" }, messages.filterEnabled) : null, h("div", { key: "aria-item-count" }, messages.total.replace("{count}", numberStringFormatter.localize(filteredItems.length.toString()))), filteredItems.length ? h("ol", { key: "aria-item-list" }, filteredItems.map((item) => h("li", null, item.label))) : null) : null;
  }
  connectObserver() {
    var _a;
  ...
  handleNudgeEvent(event) {
    ...
    const dragEl = composedPath.find((el) => (el == null ? void 0 : el.tagName) && el.matches(dragSelector));
    ...
    const referenceEl = direction === "up" && newIndex !== lastIndex || direction === "down" && newIndex === 0 ? sameParentItems[newIndex] : sameParentItems[newIndex].nextSibling;
...
var ListItem = class {
  ...
  async setFocus() {
    ...
    if (typeof focusIndex === "number") {
      ...
      if (cells[focusIndex]) {
        ...
        containerEl == null ? void 0 : containerEl.focus();
      }
      return;
    }
    containerEl == null ? void 0 : containerEl.focus();
  ...
  renderSelected() {
    ...
    }, key: "selection-container", onClick: this.handleItemClick }, h("calcite-icon", { icon: selected ? selectionMode === "multiple" ? ICONS.selectedMultiple : ICONS.selectedSingle : selectionMode === "multiple" ? ICONS.unselectedMultiple : ICONS.unselectedSingle, scale: "s" }));
  }
  renderDragHandle() {
    const { label, dragHandle, dragSelected, dragDisabled, setPosition, setSize } = this;
    return dragHandle ? h("td", {
      "aria-label": label,
      class: CSS$2.dragContainer,
      key: "drag-handle-container",
      onFocusin: this.focusCellHandle,
      role: "gridcell",
      // eslint-disable-next-line react/jsx-sort-props -- ref should be last so node attrs/props are in sync (see https://github.com/Esri/calcite-design-system/pull/6530)
      ref: (el) => this.handleGridEl = el
    }, h("calcite-handle", { disabled: dragDisabled, label, onCalciteHandleChange: this.dragHandleSelectedChangeHandler, selected: dragSelected, setPosition, setSize })) : null;
  }
  renderOpen() {
    ...
    return openable ? h("td", { class: CSS$2.openContainer, key: "open-container", onClick: this.handleToggleClick, title: tooltip }, h("calcite-icon", { icon, key: icon, scale: "s" })) : null;
  }
  renderActionsStart() {
    const { label, hasActionsStart } = this;
    return h("td", {
      "aria-label": label,
      class: CSS$2.actionsStart,
    ...
    }, h("slot", { name: SLOTS$1.actionsStart, onSlotchange: this.handleActionsStartSlotChange }));
  }
  renderActionsEnd() {
    const { label, hasActionsEnd, closable, messages } = this;
    return h("td", {
      "aria-label": label,
      class: CSS$2.actionsEnd,
    ...
    }, h("slot", { name: SLOTS$1.actionsEnd, onSlotchange: this.handleActionsEndSlotChange }), closable ? h("calcite-action", { appearance: "transparent", icon: ICONS.close, key: "close-action", label: messages.close, onClick: this.handleCloseClick, text: messages.close }) : null);
  }
  renderContentStart() {
  ...
  render() {
    ...
    return h(Host, null, h(InteractiveContainer, { disabled: this.disabled }, h("tr", {
...
var ListItemGroup = class {
  ...
  render() {
    ...
    return h(Host, null, h(InteractiveContainer, { disabled }, h("tr", { class: CSS.container, style: { "--calcite-list-item-spacing-indent-multiplier": `${visualLevel}` } }, h("td", { class: CSS.heading, colSpan: MAX_COLUMNS }, heading)), h("slot", { onSlotchange: this.handleDefaultSlotChange })));
  }
  get el() {
    return getElement(this);
  }
...
export {
  List as calcite_list,
  ListItem as calcite_list_item,
  ListItemGroup as calcite_list_item_group
};
/*! Bundled license information:

@esri/calcite-components/dist/esm/calcite-list_3.entry.js:
  (*!
   ...
   *)
*/
//# sourceMappingURL=calcite-list_3.entry-3Y2JJUTB.js.map
