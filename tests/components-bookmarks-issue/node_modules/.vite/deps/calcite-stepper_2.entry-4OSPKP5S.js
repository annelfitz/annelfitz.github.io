...
var StepBar = ({ disabled, active, complete, error, key }) => h(
  "svg",
  { class: {
    [CSS$2.stepBar]: true
  }, key },
...
var Stepper = class {
  constructor(hostRef) {
    ...
    this.handleDefaultSlotChange = (event) => {
      const items = slotChangeGetAssignedElements(event).filter((el) => (el == null ? void 0 : el.tagName) === "CALCITE-STEPPER-ITEM");
      this.items = items;
    ...
    this.currentActivePosition = void 0;
    this.elWidth = void 0;
  }
  ...
  componentDidLoad() {
    var _a;
    (_a = this.resizeObserver) == null ? void 0 : _a.observe(this.containerEl);
  ...
  render() {
    return h(Host, { "aria-label": this.messages.label, role: "region" }, h("div", { class: { container: true, [CSS$1.singleView]: this.layout === "horizontal-single" }, ref: this.setContainerEl }, this.layout === "horizontal-single" && h("div", { class: { [CSS$1.stepBarContainer]: true } }, this.items.map((item, index) => h(StepBar, { active: index === this.currentActivePosition, complete: item.complete && index !== this.currentActivePosition && !item.error, disabled: item.disabled && index !== this.currentActivePosition, error: item.error && index !== this.currentActivePosition, key: index }))), this.layout === "horizontal-single" && h("div", { class: { [CSS$1.actionContainer]: true } }, this.renderAction("start"), this.renderAction("end")), h("slot", { onSlotchange: this.handleDefaultSlotChange })));
  }
  //--------------------------------------------------------------------------
  //
  //  Event Listeners
  ...
  getEnabledStepIndex(startIndex, direction = "next") {
    ...
    while (((_a = items[newIndex]) == null ? void 0 : _a.disabled) && this.layout !== "horizontal-single") {
  ...
  renderAction(position) {
    ...
    }, compact: true, "data-position": position, disabled: currentActivePosition === 0 && isPositionStart || currentActivePosition === totalItems - 1 && !isPositionStart, icon: path, iconFlipRtl: true, id, onClick: this.handleActionClick, scale: this.scale, text: isPositionStart ? this.messages.previousStep : this.messages.nextStep }) : null;
  }
...
var StepperItem = class {
  constructor(hostRef) {
    ...
    this.handleItemClick = (event) => {
      if (this.disabled || this.layout === "horizontal" && event.composedPath().some((el) => {
        var _a;
        return (_a = el.classList) == null ? void 0 : _a.contains("stepper-item-content");
      })) {
        return;
      }
      this.emitUserRequestedItem();
  ...
  render() {
    return h(Host, { "aria-current": this.selected ? "step" : "false", onClick: this.handleItemClick, onKeyDown: this.keyDownHandler, tabIndex: this.disabled ? -1 : 0 }, h(InteractiveContainer, { disabled: this.disabled }, h("div", { class: CSS.container }, this.complete && h("span", { "aria-live": "polite", class: CSS.visuallyHidden }, this.messages.complete), h("div", {
    ...
    }, this.icon ? this.renderIcon() : null, this.numbered ? h("div", { class: CSS.stepperItemNumber }, this.renderNumbers(), ".") : null, h("div", { class: CSS.stepperItemHeaderText }, h("span", { class: CSS.stepperItemHeading }, this.heading), h("span", { class: CSS.stepperItemDescription }, this.description))), h("div", { class: CSS.stepperItemContent }, h("slot", null)))));
  }
  //--------------------------------------------------------------------------
  //
  //  Event Listeners
  ...
  async setFocus() {
    ...
    (_a = this.layout === "vertical" ? this.el : this.headerEl) == null ? void 0 : _a.focus();
  }
  renderIcon() {
    let path = "circle";
  ...
  getItemPosition() {
    var _a;
    return Array.from((_a = this.parentStepperEl) == null ? void 0 : _a.querySelectorAll("calcite-stepper-item")).indexOf(this.el);
  }
  renderNumbers() {
    numberStringFormatter.numberFormatOptions = {
...
export {
  Stepper as calcite_stepper,
  StepperItem as calcite_stepper_item
};
/*! Bundled license information:

@esri/calcite-components/dist/esm/calcite-stepper_2.entry.js:
  (*!
   ...
   *)
*/
//# sourceMappingURL=calcite-stepper_2.entry-4OSPKP5S.js.map
