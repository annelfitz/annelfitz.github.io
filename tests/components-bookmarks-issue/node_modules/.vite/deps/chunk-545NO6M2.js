...
var a = { [o.TimeZoneNotRecognized]: "Timezone identifier has not been recognized." };
...
var m = class _m {
  ...
  static fromParts(e = 0, t2 = 1, r3 = 1, s2 = 0, i2 = 0, o3 = 0, a3 = 0, d2) {
    if (isNaN(e) || isNaN(t2) || isNaN(r3) || isNaN(s2) || isNaN(i2) || isNaN(o3) || isNaN(a3))
      return null;
    const l = DateTime.local(e, t2).daysInMonth;
    let f = DateTime.fromObject({ day: c2(r3, 1, l), year: e, month: c2(t2, 1, 12), hour: c2(s2, 0, 23), minute: c2(i2, 0, 59), second: c2(o3, 0, 59), millisecond: c2(a3, 0, 999) }, { zone: h(d2) });
    return f = f.plus({ months: u(t2, 1, 12), days: u(r3, 1, l), hours: u(s2, 0, 23), minutes: u(i2, 0, 59), seconds: u(o3, 0, 59), milliseconds: u(a3, 0, 999) }), new _m(f);
  }
  ...
  static dateJSAndZoneToArcadeDate(e, t2 = "system") {
    const r3 = h(t2);
    return new _m(DateTime.fromJSDate(e, { zone: r3 }));
  }
  static unknownEpochToArcadeDate(e) {
    return new _m(DateTime.fromMillis(e, { zone: t.instance }));
  }
  static unknownDateJSToArcadeDate(e) {
    return new _m(DateTime.fromMillis(e.getTime(), { zone: t.instance }));
  }
  static epochToArcadeDate(e, t2 = "system") {
    const r3 = h(t2);
    return new _m(DateTime.fromMillis(e, { zone: r3 }));
  }
...
function h(e, n4 = true) {
  ...
  if (!a3.isValid) {
    if (n4)
      throw new d(o.TimeZoneNotRecognized);
    return null;
  }
  return a3;
}
...
function o2(t2) {
  ...
  let e = "";
  const a3 = /(\[[^\[]*\])|(\\)?([Hh]mm(ss)?|Mo|MM?M?M?|Do|DDDo|DD?D?D?|ddd?d?|do?|w[o|w]?|W[o|W]?|Qo?|N{1,5}|YYYYYY|YYYYY|YYYY|YY|y{2,4}|yo?|gg(ggg?)?|GG(GGG?)?|e|E|a|A|hh?|HH?|kk?|mm?|ss?|S{1,9}|x|X|zz?|ZZ?|.)/g;
  for (const r3 of t2.match(a3) || [])
    switch (r3) {
      case "D":
        e += "d";
        break;
...
var n = class _n {
  ...
  toFormat(t2 = null, r3 = true) {
    ...
    return m.dateTimeToArcadeDate(s2).toFormat(t2, { locale: c(), numberingSystem: "latn" });
  }
  toArcadeDate() {
  ...
  plus(t2, e) {
    return _n.fromDateTime(this.toUTCDateTime().plus({ [t2]: e }));
  }
  toUTCDateTime() {
  ...
  static fromString(t2, e = null) {
    ...
    for (const s2 of a3) {
      ...
      if (a4.isValid)
        return new _n(a4.year, a4.month, a4.day);
    }
    return null;
  }
...
function s(r3) {
  ...
  switch (r3.type) {
    ...
    case "polyline":
      return r3.extent;
    case "point":
      return new w({ xmin: r3.x, ymin: r3.y, xmax: r3.x, ymax: r3.y, spatialReference: r3.spatialReference });
    case "extent":
      return r3;
  }
  return null;
...
var n3 = class _n {
  ...
  toFormat(t2 = null) {
    ...
    if (!(t2 = i(t2)))
      return "";
    return DateTime.local(1970, 1, 1, this._hour, this._minute, this._second, this._millisecond).toFormat(t2, { locale: c(), numberingSystem: "latn" });
  }
  toNumber() {
  ...
  static fromString(t2, s2 = null) {
    ...
    for (const i2 of r3) {
      ...
      if (s3.isValid)
        return new _n(s3.hour, s3.minute, s3.second, s3.millisecond);
    }
    return null;
  }
  plus(t2, s2) {
    switch (t2) {
      ...
      case "seconds":
      case "milliseconds":
        return _n.fromDateTime(this.toUTCDateTime().plus({ [t2]: s2 }));
    }
    return null;
  }
  toUTCDateTime() {
...
//# sourceMappingURL=chunk-545NO6M2.js.map
